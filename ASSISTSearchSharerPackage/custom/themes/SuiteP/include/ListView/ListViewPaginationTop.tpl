					...
					<td nowrap="nowrap" class='paginationActionButtons'>
						...
						<script>
							...
							function assist_show_shareable_link(){
								...
								mb.setTitle('{/literal}{$APP.LBL_SEARCH_SHARER_TITLE}{literal}');
								currentQuery = JSON.parse(currentQuery);
								if(!currentQuery){

									mb.setBody('{/literal}{$APP.LBL_SEARCH_SHARER_NO_QUERY}{literal}');
									mb.show();
									return;
								}
								for(prop in currentQuery){
									url += "&"+prop+"="+currentQuery[prop];
								}

								mb.setBody("<button onclick='navigator.clipboard.writeText(\""+url+"\").then(function() {alert(\"{/literal}{$APP.LBL_SEARCH_SHARER_CLIPBOARD_SET}{literal}\");});' class='btn btn-primary'>{/literal}{$APP.LBL_SEARCH_SHARER_COPY}{literal}</button><h6>{/literal}{$APP.LBL_SEARCH_SHARER_FULL_URL}{literal}</h6><textarea style='width: 100%;' rows=10>"+url+"</textarea>");
								mb.show();
							}
							{/literal}
						</script>
