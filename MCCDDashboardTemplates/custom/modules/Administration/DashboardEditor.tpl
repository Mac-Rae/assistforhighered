...
<form id="ConfigureSettings" name="ConfigureSettings" enctype='multipart/form-data' method="POST"
    ...
    <table style="width: 50%;" border="0" cellspacing="1" cellpadding="0" class="edit view">
        <tr>
            <th>{$MOD.LBL_MCCDDASHBOARDEDITOR_TARGET}</th>
            <th>{$MOD.LBL_MCCDDASHBOARDEDITOR_DASHBOARD}</th>
        </tr>
        <tr id="buttonRow">
            <td>
                <button class="button" id="newLineButton" type="button" value="{$MOD.LBL_MCCDDASHBOARDEDITOR_NEW_LINE}">{$MOD.LBL_MCCDDASHBOARDEDITOR_NEW_LINE}</button>
            </td>
        </tr>
    </table>
...
<script>
    ...
    function applyAll(e){
        var target = $(e.target);
        if(!confirm('{/literal}{$MOD.LBL_MCCDDASHBOARDEDITOR_APPLY_ALL_CONFIRM}{literal}')){
            return;
        }
        $.post('index.php',{
            'module' : 'Administration',
            'to_pdf' : 1,
            'action' : 'mccdApplyDashboardTemplates',
            'role' : target.closest('tr').find('.target_role').val(),
            'dashboard' : target.closest('tr').find('.dashboard').val()
        },function(d){
            if(d['success']){
                alert('{/literal}{$MOD.LBL_MCCDDASHBOARDEDITOR_APPLY_ALL_SUCCESS}{literal}')
            }else{
                alert('{/literal}{$MOD.LBL_MCCDDASHBOARDEDITOR_APPLY_ALL_ERROR}{literal}' + d['error']);
            }
        },'JSON');
    }
...
<table style="display: none;">
    <tr id="row_template">
        <td>
            <select
                    class='target_role' name='mccd_dashboardtemplates[target_role][]'>
                {$roleOptions}
            </select>
        </td>
        <td>
            <select
                    class='dashboard' name='mccd_dashboardtemplates[dashboard][]'>
                {$dashboardOptions}
            </select>
        </td>
        <td>
            <input
                    ...
                    type="button"
                    value="{$MOD.LBL_MCCDDASHBOARDEDITOR_APPLY_ALL}">
        </td>
        <td>
            <input
