...
class ElasticSearchIndexer extends AbstractIndexer
{
    ...
    /** @var string|false The latest modified date of the last batch indexed  */
    private $latestModified = false;
    ...
    public function indexBeans($module, array $beans)
    {
        ...
        if ($this->latestModified) {
            $this->putMeta($module, ['last_index' => $this->latestModified]);
        }
    }
    ...
    private function indexBatch(string $module, array $beans): void
    {
        ...
        foreach ($beans as $key => $bean) {
            ...
            $this->latestModified = $bean->last_modified;
        }
    ...
    {
        $params = $this->documentifier->documentify($bean);
        if(array_key_exists('converted',$params)){
            unset($params['converted']);
        }
        return $params;
    }
