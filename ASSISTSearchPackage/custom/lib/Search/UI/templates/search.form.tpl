...
<style>
    {literal}
    .global_search_enable_modules_panel{
            		...
            		<td colspan="6" style="padding-left: 20px;" nowrap="">
                		<div id="inlineGlobalSearch">
                    		<table style="margin-bottom:0px;" cellspacing="0" cellpadding="0" border="0">
                                ...
                                <tr>
                                    <td>
                                        <div class="global_search_enable_modules_panel">
                                            {foreach from=$module_list item=module_name key=module_key}
                                                <label>{$module_name}&nbsp;<input {if $module_key|in_array:$selectedModules}checked="checked"{/if} type="checkbox" name="enable_modules[]" value="{$module_key}"/></label>
                                            {/foreach}
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        ...
                                        <label for="search-total-max" class="text-muted">{$APP.LBL_SEARCH_RESULTS_TOTAL_MAX}</label>
                                        {html_options options=$totalSizeOptions selected=$searchTotalMax id="search-total-max" name="search-total-max"}
                                        &nbsp;&nbsp;
                                        <input type="hidden" name="search-query-from" value="{$searchQueryFrom}">
